...
## COVID-19 INDIA - EXPLORATORY ANALYSIS
...
rm(Compare,TestsDaily_Tests,p,TestTill_DayBefore,TodayTest,YesterdayTest,col,cols,popup)
...
Comparision of few states on Positivity Rate; Increased Testing Rate vs Confrimed Increasing Rate over Recent Days
</br>
</br>
```{r echo=FALSE,fig.width=9.5,fig.height=7.5,message=FALSE,warning=FALSE,error=FALSE,eval=TRUE}

PlottingFrame<-subset(PlottingFrame,PlottingFrame$states_daily.date==Sys.Date()-2 | PlottingFrame$states_daily.date==Sys.Date()-4)

PlottingFrame<-melt(PlottingFrame,id=c("states_daily.date","State"))
PlottingFrame<-dcast(PlottingFrame, State ~ states_daily.date + variable)
PlottingFrame$ConfirmedIncr<-round(round(((PlottingFrame[,4]-PlottingFrame[,2])/PlottingFrame[,2])*100,2)/2,2)
PlottingFrame$TestIncr<-round(round(((PlottingFrame[,5]-PlottingFrame[,3])/PlottingFrame[,3])*100,2)/2,2)
PlottingFrame$LatestPositivityRate<-round(((PlottingFrame[,4])/PlottingFrame[,5])*100,2)

PlottingFrame<-subset(PlottingFrame,PlottingFrame[,4]>10 & PlottingFrame$LatestPositivityRate > 0)
...
fig <- plot_ly(
  type = 'table',
  header = list(
    values = c(names(PlottingFrame)),
    align = c('left', rep('center', ncol(PlottingFrame))),
  ...
  cells = list(
    values = rbind(
      t(as.matrix(unname(PlottingFrame)))
    ),
    align = c('left', rep('center', ncol(PlottingFrame))),
